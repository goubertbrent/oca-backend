...
var DATESELECT_OPTION_ADD_TO_CALENDER = "add_to_calender";
var DATESELECT_OPTION_REMIND_ME = "remind_me";
...
var getAdminCalendars = function() {
...
var createGuestDetailListItem = function(guest) {
...
var getNextStartAndEndTime = function(event, now, full_end_date) {
    ...
    if (full_end_date) {
        return {"start": event.start_dates[event.start_dates.length - 1], "end": event.end_dates_timestamps[event.end_dates_timestamps.length - 1]};
    } else {
        return {"start": event.start_dates[event.start_dates.length - 1], "end": event.end_dates[event.end_dates.length - 1]};
    }
};

var getUpcommingStartAndEndDates = function(event, now, full_end_date, include_yesterday) {
    var checkdate = now;
...
var hideRemindMePopupOverlay = function() {
    $("#remind-me-popup").popup("close");  // needs to be double for first close
    $("#remind-me-popup").popup("close");
};

var showRemindMePopupOverlay = function(event, eventStartEnd) {
...
var hideDateselectPopupOverlay = function() {
    $("#dateselect-popup").popup("close");  // needs to be double for first close
    $("#dateselect-popup").popup("close");
};

var showDateselectPopupOverlay = function(event, option_type) {
    $("#dateselect-popup-options").empty();
    ...
    $("#dateselect-popup").popup("open", {positionTo: 'window'});
    $('#detail').trigger('create');
};

var hideEventRemovePopupOverlay = function() {
    $("#event-remove-popup").popup("close");  // needs to be double for first close
    $("#event-remove-popup").popup("close");
};
...
var onRogerthatReady = function() {
    ...
    var addToCalender = function(event, eventStartEnd) {
        var eventDate = new Date(eventStartEnd.start.year, eventStartEnd.start.month - 1, eventStartEnd.start.day, eventStartEnd.start.hour, eventStartEnd.start.minute);
        var eventStart = eventDate.getTime() / 1000;
    ...
    $(document).on("click", ".dateSelectOptionsSelector", function() {
        console.log("#detail .dateSelectOptionsSelector");
        var event = $("#detail").data("event");
        ...
        console.log("dateSelectOptionsSelector type: " + option_type);
        if (option_type === DATESELECT_OPTION_ADD_TO_CALENDER) {
            addToCalender(event, selectedEvent);
        } else {
            showRemindMePopupOverlay(event, selectedEvent);
        }
    });
    ...
    $(document).on("click", ".closeDateSelectPopup", function() {
        hideDateselectPopupOverlay();
    });
    ...
    $(document).on("click", ".calendar-save", function() {
        var disabled = [];
        $('#calendar-checkboxes input[type=checkbox]:not(:checked)').each(function() {
    ...
    $(document).on("click", ".gotoBroadcast", function() {
        ...
        var calendars = filter(rogerthat.service.data.solutionCalendars, function (calendar, i) {
...
var parseDateToEventDateFromTill = function(from, until) {
...
var resetGuestDetails = function(status) {
...
var loadGuestsDetails = function(eventId, status) {
    $.mobile.loading('hide');
    var r = guestsDict[eventId];
...
var loadEvents = function() {
    ...
    var now = (new Date().getTime()) / 1000;
    var checkdate = now - DAY;
    events = filter(rogerthat.service.data.solutionEvents, function (event, i) {
