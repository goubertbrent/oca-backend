...
from solutions.common.to.news import NewsBroadcastItemTO, NewsBroadcastItemListTO, \
...
def publish_item(service_identity_user, app_id, host, is_free_regional_news, coupon, should_save_coupon,
                 broadcast_on_facebook, broadcast_on_twitter, facebook_access_token, **kwargs):
...
def send_news_for_review(city_service, service_identity_user, app_id, host, is_free_regional_news, coupon,
                         broadcast_on_facebook, broadcast_on_twitter, facebook_access_token,
                         **kwargs):
...
@returns(NewsBroadcastItemTO)
@arguments(service_identity_user=users.User, title=unicode, message=unicode, broadcast_type=unicode,
           ...
           tag=unicode)
def put_news_item(service_identity_user, title, message, broadcast_type, image, action_button, news_type,
                  qr_code_caption, app_ids, scheduled_at, news_id=None, broadcast_on_facebook=False,
    ...
    sticky = False
    # This increases the sort priority of city news
        ...
        for app_id in app_ids:
            ...
            if city_service and city_service != service_user:
                ...
                if city_app_profile.review_news:
                    ...
                    send_news_for_review(city_service, service_identity_user, app_id, host, is_free_regional_news,
                                         coupon, broadcast_on_facebook, broadcast_on_twitter, facebook_access_token,
                                         **city_kwargs)
    ...
    with users.set_user(service_user):
        return publish_item(service_identity_user, si.app_id, host, is_free_regional_news, coupon, should_save_coupon,
                            broadcast_on_facebook, broadcast_on_twitter, facebook_access_token, **kwargs)
