...
from rogerthat.to.forms import DynamicFormTO, FormStatisticsTO
from rogerthat.to.service import UserDetailsTO
from solutions.common.bizz.forms import create_form, get_form, update_form, get_tombola_winners
from solutions.common.to.forms import OcaFormTO


@rest('/common/forms', 'get', read_only_access=True)
@returns([DynamicFormTO])
@arguments()
def rest_list_forms():
    return service_api.list_forms()


@rest('/common/forms/<form_id:[^/]+>', 'get', read_only_access=True)
@returns(OcaFormTO)
...
@rest('/common/forms', 'post', type=REST_TYPE_TO)
@returns(OcaFormTO)
...
@rest('/common/forms/<form_id:[^/]+>', 'put', read_only_access=True, type=REST_TYPE_TO)
@returns(OcaFormTO)
...
@rest('/common/forms/<form_id:[^/]+>/statistics', 'get', read_only_access=True)
@returns(FormStatisticsTO)
@arguments(form_id=(int, long))
def rest_get_form_statistics(form_id):
    return service_api.statistics(form_id)


@rest('/common/forms/<form_id:[^/]+>/test', 'post', read_only_access=True)
@returns()
...
@rest('/common/forms/<form_id:[^/]+>/tombola/winners', 'get', read_only_access=True)
