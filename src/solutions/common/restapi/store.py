...
from shop.models import Order, OrderItem, Product, RegioManagerTeam, Contact
from shop.to import OrderItemTO, ProductTO, ShopProductTO, OrderItemReturnStatusTO, BoolReturnStatusTO
from solutions import translate, SOLUTION_COMMON
from solutions.common.bizz import SolutionModule
from solutions.common.bizz.store import get_payconiq_info, process_order
from solutions.common.dal import get_solution_settings
from solutions.common.to import StorePaymentMethods
...
@arguments()
def rest_pay_order():
    return process_order(True)


@rest('/common/store/payment-methods', 'get', read_only_access=True)
@returns(StorePaymentMethods)
@arguments()
def get_payment_methods():
    service_user = users.get_current_user()
    customer = get_customer(service_user)
    return StorePaymentMethods(credit_card=True, payconiq=customer and customer.country == 'BE')


@rest('/common/store/payconiq', 'get', read_only_access=False)
@returns(dict)
@arguments()
def rest_get_payconiq_info():
    service_user = users.get_current_user()
    return get_payconiq_info(service_user)
