...
@returns([App])
@arguments(customer=(Customer, NoneType))
def get_available_apps_for_customer(customer):
    if not customer:
        return []
    available_apps = list(App.get(
        map(App.create_key, (a for a in customer.sorted_app_ids if a != App.APP_ID_OSA_LOYALTY))))
