...
from rogerthat.bizz.news.influx import get_news_item_time_statistics, get_basic_news_item_statistics, \
    get_news_item_statistics_per_app
...
from rogerthat.to.news import NewsActionButtonTO, NewsItemTO, NewsItemListResultTO, \
    ...
    NewsItemTimeStatisticsTO, NewsItemStatisticsPerApp
...
def get_basic_statistics(ids, service_identity=None):
    service_user = users.get_current_user()
    service_identity_user = get_and_validate_service_identity_user(service_user, service_identity)
    news.get_and_validate_news_items(ids, service_identity_user)
    return get_basic_news_item_statistics(ids)


@service_api(function=u'news.get_app_statistics', silent_result=True)
@returns([NewsItemStatisticsPerApp])
@arguments(ids=[(int, long)], service_identity=unicode, total_only=bool)
def get_app_statistics(ids, service_identity=None, total_only=True):
    """Setting total_only to True reduces processing time and bandwidth usage, so it'll be faster.
    Obviously this also means you only get totals, and no gender / age stats."""
    service_user = users.get_current_user()
    service_identity_user = get_and_validate_service_identity_user(service_user, service_identity)
    news.get_and_validate_news_items(ids, service_identity_user)
    return get_news_item_statistics_per_app(ids, total_only)
...
def _get_service_identity(service_identity_user):
    return get_service_identity(service_identity_user)
...
@arguments(sticky=bool, sticky_until=(int, long), title=unicode, message=unicode, image=unicode, news_type=(int, long),
           ...
           community_ids=[(int , long)], service_identity=unicode, target_audience=NewsTargetAudienceTO,
...
def publish(sticky=MISSING, sticky_until=MISSING, title=MISSING, message=MISSING, image=MISSING, news_type=MISSING,
            ...
            scheduled_at=MISSING, flags=MISSING, news_id=None, community_ids=MISSING, service_identity=None,
            target_audience=None, tags=None, media=MISSING, locations=None, group_type=MISSING,
    ...
    news_item = news.put_news(service_identity_user, sticky, sticky_until, title, message, image, news_type,
                              action_buttons, qr_code_content, qr_code_caption, community_ids, scheduled_at, flags,
                              news_id,
                              target_audience, tags, media, locations, group_type, group_visible_until,
                              timestamp,
                              accept_missing=True)
        ...
        for button in news_item.buttons:
            for btn in action_buttons:
                if button.id == btn.id and button.action.startswith('poke://'):
                    ...
                    break
    service_profile = get_service_profile(service_user)
    si = _get_service_identity(service_identity_user)
    server_settings = get_server_settings()
    share_base_url = get_news_share_base_url(server_settings.webClientUrl, community_id=service_profile.community_id)
    share_url = get_news_share_url(share_base_url, news_item.id)
...
def list_groups(language=None):
    service_user = users.get_current_user()
    return news.groups.get_group_types_for_service_user(service_user, language=language or DEFAULT_LANGUAGE)
