...
class AuthToken(db.Model):
    ...
    auth_token = db.StringProperty(indexed=False)

    @staticmethod
    def key_name(service, token):
        d = hashlib.sha256()
        d.update(service)
        d.update(token)
        return d.hexdigest()
...
class OAuthClient(object):
    ...
    def make_async_request(self, url, token="", secret="", additional_params=None,
                                                 protected=False, method=urlfetch.GET, headers={}, body=None):
        """Make Request.
        ...
        if method == urlfetch.POST and body:
            url = "%s?%s" % (url, payload)
            payload = body
    ...
    def make_request(self, url, token="", secret="", additional_params=None,
        ...
        return self.make_async_request(url, token, secret, additional_params,
                                                                     protected, method, headers, body).get_result()
    ...
    def _get_auth_token(self):
        ...
        key = AuthToken.key_name(self.service_name, auth_token)
        auth_model = AuthToken(key_name=key, secret=auth_secret)
        auth_model.put()

        # Add the secret to memcache as well.
        memcache.set(key, auth_model, time=20 * 60)  # @UndefinedVariable
