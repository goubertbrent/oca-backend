...
# Copied and updated based on https://github.com/erwan-lemonnier/cloud-storage-image-resizer/blob/master/cloudstorageimageresizer.py


class ImageResizerException(Exception):
    pass


class InvalidParameterException(ImageResizerException):
    pass


class CantFetchImageException(ImageResizerException):
    pass


class RTFMException(ImageResizerException):
    pass
...
class ImageResizer(object):
    ...
    def __init__(self, client, bucket_name=None):
        ...
        if not client or gcloud_client_str not in str(type(client)):
            msg = "Expected an instance of google storage Client, got a %s" % client
    ...
    def __load_image(self, image_in_bytes):
        """Instantiate a Pillow image, set its exif tags if any and do some pre-processing"""
    ...
    def orientate(self):
        ...
        logger.debug("Image has exif tags: %s" % self.exif_tags)

        # No exif orientation?
        if 'orientation' not in self.exif_tags:
            logger.debug("No exif orientation known for this image")
            return self
        ...
        logger.debug("Applying exif orientation %s to image" % angle)
    ...
    def resize_if_larger_and_keep_ratio(self, width=None, height=None):
        ...
        if width and height and cur_width > width and cur_height > height:
            logger.debug("Resizing image from (%s, %s) to (%s, %s)" % (cur_width, cur_height, width, height))
            self.image.thumbnail((width, height), Image.ANTIALIAS)
        elif width and cur_width > width:
            logger.debug("Resizing image width from %s to %s" % (cur_width, width))
            self.image.thumbnail((width, cur_height), Image.ANTIALIAS)
        elif height and cur_height > height:
            logger.debug("Resizing image height from %s to %s" % (cur_height, height))
    ...
    def resize(self, width=None, height=None, progressive=False):
        ...
        clone = ImageResizer(self.client, bucket_name=self.bucket_name)
        logger.debug("Resizing image from (%s, %s) to (%s, %s)" % (cur_width, cur_height, to_width, to_height))
    ...
    def crop(self, width=None, height=None):
        ...
        x height pixels. The croped section must be smaller than the current image. Both width and height must be set"""
        ...
        logger.debug("Cropping image of size (%s, %s) into a box of size (%s, %s, %s, %s)" % (width, height, left, upper, right, lower))
        clone = ImageResizer(self.client, bucket_name=self.bucket_name)
    ...
    def make_round(self):
        """Take a square PNG image and make its corner transparent so it looks like a circle"""
        w = self.image.width
        h = self.image.height
    ...
    def store_and_return_blob(self, bucket_name=None, key_name=None, metadata=None, quality=95, encoding='PNG',
                              progressive=True):
        """Store the image into the given bucket (or defaults to the bucket passed to the constructor), with the given key name.
        ...
        logger.debug("Storing image into bucket %s/%s" % (bucket_name, key_name))
    ...
    def store_and_return_url(self, in_bucket=None, key_name=None, metadata=None, quality=95, public=True,
                             encoding='PNG', progressive=True):
        """Store the loaded image into the given bucket with the given key name. Tag it
        with metadata if provided. Optionally make the Image public. Return its url."""
        ...
        return blob.public_url, blob.content_type, blob.size
