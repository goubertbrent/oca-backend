...
import {
  AddVoucherAction, AddVoucherFailedAction, AddVoucherSuccessAction,
  CirkloActionTypes,
  ...
  GetMerchantsAction,
  GetMerchantsCompleteAction, GetMerchantsFailedAction,
...
export class CirkloEffects {
  ...
  deleteConfirm$ = createEffect(() => this.actions$.pipe(
    ...
    switchMap(({ payload }) => from(this.showConfirmDialog(this.translate.instant('app.oca.confirm_delete_voucher'))).pipe(
  ...
  private async showConfirmDialog(message: string) {
    const alert = await this.alertController.create({
      header: this.translate.instant('app.oca.confirmation'),
      message,
      buttons: [{
