...
class Test(mc_unittest.TestCase):
    ...
    def _prepareService(self, email):
        ...
        def trans(service_profile_key, service_identity_key):
            service_profile, service_identity = db.get([service_profile_key, service_identity_key])
            service_profile.supportedLanguages = ["en", "fr", "nl"]

            service_profile_attrs = [
                "aboutMenuItemLabel", "messagesMenuItemLabel", "shareMenuItemLabel", "callMenuItemLabel"]
            ...
            service_identity_attrs = ["name", "qualifiedIdentifier", "description", "descriptionBranding", "menuBranding",
                                      "mainPhoneNumber", "callMenuItemConfirmation", "homeBrandingHash"]
            for attr in service_identity_attrs:
                setattr(service_identity, attr, attr)
            service_identity.put()

            return service_profile, service_identity

        service_profile, service_identity = db.run_in_transaction(trans, service_profile.key(), service_identity.key())
