...
class Test(mc_unittest.TestCase):
    ...
    def test_serialize_with_default(self):
        ...
        result.value.message = u'message'
        result.value.tag = u'tag'
        result.value.attachments = []
        result.value.step_id = None

        serialize_value(result, *get_type_details(MessageAcknowledgedCallbackResultTO, result), skip_missing=True)

        self.assertEqual(TextWidget.keyboard_type.default, result.value.form.widget.keyboard_type)
    ...
    def testArgumentWithMultiplePossibleTypes(self):

        @returns()
        @arguments(s=(unicode, [unicode], [(int, long)]))
        def test(s):
            pass
        ...
        test([1, 2, 3L])
        test([1L, 2L, 3, 4L])
        self.assertRaises(ValueError, lambda: test(True))
        self.assertRaises(ValueError, lambda: test(1))
        self.assertRaises(ValueError, lambda: test(1.0))
        self.assertRaises(ValueError, lambda: test([1.0, 2.0]))
        self.assertRaises(ValueError, lambda: test([None]))
        self.assertRaises(ValueError, lambda: test(['a', 2]))
